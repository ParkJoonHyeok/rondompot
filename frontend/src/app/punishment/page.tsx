...
export default function PunishmentPage() {
  const FORM_STORAGE_KEY = "randompot:punishment:form";
  const HISTORY_STORAGE_KEY = "randompot:punishment:history";
  ...
  const [history, setHistory] = useState<PunishmentHistoryItem[]>([]);
  const [error, setError] = useState("");
  const [loading, setLoading] = useState(false);
  const [copyFeedback, setCopyFeedback] = useState("");
  const [pageLinkFeedback, setPageLinkFeedback] = useState("");
  const titleInputId = "punishment-title";
  const itemsTextareaId = "punishment-items";
  const itemsHintId = "punishment-items-hint";
  ...
  useEffect(() => {
    ...
    window.sessionStorage.setItem(LAST_FEATURE_STORAGE_KEY, "punishment");
  ...
  useEffect(() => {
    ...
    try {
      const parsed = JSON.parse(stored) as PunishmentHistoryItem[];
  ...
  function buildResultText(value: PunishmentDrawResponse) {
    const lines = ["[랜덤팟 벌칙 뽑기]"];
    if (value.title.trim()) {
  ...
  async function handleCopyHistory(item: PunishmentHistoryItem) {
    try {
  ...
  async function handleShareHistory(item: PunishmentHistoryItem) {
  ...
  return (
    <main style={{ width: "100%", boxSizing: "border-box", padding: "16px 14px 24px", maxWidth: "520px", margin: "0 auto" }}>
      <h1 style={{ marginBottom: "14px" }}>벌칙</h1>
      <p style={{ marginTop: 0, color: "#555" }}>제목과 벌칙 항목을 입력하면 결과를 바로 랜덤으로 뽑을 수 있어요.</p>
      <button
        type="button"
        onClick={() => {
